...
#if defined STM32F2XX || defined STM32F4XX || defined STM32F0XX || defined STM32F1XX
// =========================== Constants and Types =============================
...
enum ADCDiv_t {
    adcDiv2 = (uint32_t)(0b00 << 14),
    adcDiv4 = (uint32_t)(0b01 << 14),
    adcDiv6 = (uint32_t)(0b10 << 14),
    adcDiv8 = (uint32_t)(0b11 << 14),
};
#else
enum ADCDiv_t {
...
#if defined STM32F1XX
#define VREF_VOLTAGE_MV     1200
class Adc_t {
private:
    uint16_t IBuf[ADC_SEQ_LEN];
    void SetupClk(ADCDiv_t Div) { RCC->CFGR = (RCC->CFGR & ~RCC_CFGR_ADCPRE) | (uint32_t)Div; }
    void SetSequenceLength(uint32_t ALen);
    void SetChannelSampleTime(uint32_t AChnl, AdcSampleTime_t ASampleTime);
    void SetSequenceItem(uint8_t SeqIndx, uint32_t AChnl);
    void StartConversion() { ADC1->CR2 |= ADC_CR2_SWSTART; }
public:
    void EnableVRef()  { ADC1->CR2 |= (uint32_t)ADC_CR2_TSVREFE; }
    void DisableVRef() { ADC1->CR2 &= (uint32_t)(~ADC_CR2_TSVREFE); }
    uint32_t GetVDAmV(uint32_t VrefADC) { return ((ADC_VREFINT_CAL * 3000UL) / VrefADC); }
    void Init();
    void StartMeasurement();
