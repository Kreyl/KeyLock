...
//            Printf("VRef_adc=%u\r", VRef_adc);
            ...
            for(int i=0; i<ADC_CHANNEL_CNT; i++) {
                ...
                uint32_t Vmv = (2UL * Vadc * 1200UL) / VRef_adc;   // Resistor divider
...
void Adc_t::Init() {
    ...
    while(BitIsSet(ADC1->CR2, ADC_CR2_RSTCAL)) {
        if(cnt++ >= 630000) {
            Printf("ADC rst calib fail\r");
            return;
        }
    }
    ADC1->CR2 |= ADC_CR2_CAL;   // Start calibration
    while(BitIsSet(ADC1->CR2, ADC_CR2_CAL)) {
        if(cnt++ >= 630000) {
